...
const InvoiceFormSP = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Budget validation - but don't expose budget details to SCM users
      try {
        const validationResponse = await axios.post(
          `${process.env.REACT_APP_API_URL}/invoice/validate-budget-scm`,
          {
        ...
        if (validationError.response?.data?.message) {
          alert(validationError.response.data.message);
        } else {
          alert(
      ...
      <Box sx={{ p: 4, backgroundColor: "#f8f9fa" }}>
        ...
        >
          ...
          <Grid container spacing={3} mb={3}>
            ...
            <Grid item xs={12} sm={4}>
              <TextField
                label="Selected MWO ID"
                value={selectedWorkOrder?.mwo_id || ""}
                InputProps={{
                  readOnly: true,
